...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	if (gUseHalfLambert)
	{
		diffuseColor *= 0.5f;
		diffuseColor += 0.5f;
		diffuseColor = sqrt(diffuseColor);
	}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	{
		float fresnelFallof = dot(normal, viewDirection);
		fresnelFallof = clamp( abs(fresnelFallof), 0, 1);
		fresnelFallof = 1- fresnelFallof;
		fresnelFallof = pow(fresnelFallof, gFresnelPower);
		fresnelFallof = fresnelFallof * gFresnelMultiplier;
		float fresnelMask = 1 -clamp(dot(float3(0,-1,0), normal), 0,1);
		fresnelMask = gFresnelHardness * fresnelMask;
		float fresnel = fresnelFallof * fresnelMask;
		
		if (gUseTextureEnvironment)
			environmentColor = environmentColor * fresnel;
		else
			environmentColor = gColorFresnel * fresnel;
	}
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	if (gUseTextureEnvironment)
	{
		...
		reflectedVector *= gReflectionStrength;
		refractedVector *= gRefractionStrength;
		...
		environmentColor = lerp(reflectedColor, refractedColor, gRefractionIndex);
	}
