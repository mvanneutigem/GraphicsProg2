...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
    ...
    topLeft = float3(-0.5f*size, 0.5f*size, 0);
    topRight = float3(0.5f * size, 0.5f * size, 0);
    bottomLeft = float3(- 0.5f * size, -0.5f * size, 0);
    bottomRight = float3(0.5f * size,  -0.5f * size, 0);
