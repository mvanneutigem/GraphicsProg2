...
>;
float m_AmountOfCrumble<
	string UIName = "amount of crumble";
	string UIWidget = "Slider";
	float UIMin = 0.0f;
	float UIMax = 100.0f;
	float UIStep = 1.0f;
...
void SplitFragmentInTwo(triangle VS_DATA vertices[3], float3 explodedPosition[3], GS_DATA outVertices[16], inout TriangleStream<GS_DATA> triStream, bool reverseWinding = true)
{
	...
	if (reverseWinding)
	{
		...
		outVertices[2].TexCoord = texCoord3;//still need to interpolate texcoord && normals
		...
		outVertices[4].TexCoord = texCoord1;//still need to interpolate texcoord && normals
...
void CreateFragment(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	float offset1 = (m_Timer * -0.01f*m_FallMultiply*abs( m_VoxelGridSize + (m_Gravity*(m_VoxelGridSize-k1)) + m_AmountOfCrumble*(j1 - l1 * l1)));
	float offset2 = (m_Timer * -0.01f*m_FallMultiply*abs( m_VoxelGridSize + (m_Gravity*(m_VoxelGridSize-k2)) + m_AmountOfCrumble*(j2 - l2 * l2)));
	float offset3 = (m_Timer * -0.01f*m_FallMultiply*abs( m_VoxelGridSize + (m_Gravity*(m_VoxelGridSize-k3)) + m_AmountOfCrumble*(j3 - l3 * l3)));
	...
	else
	{
		...
		else
		{
			...
			//TODO: classify middle frag --> too big a calculation would cause so much lag so for now all on one set fragment
			int middleX = ((v4t1.x + v4t2.x + v4t3.x) /3.0f)*1000;
			int middleY = ((v4t1.y + v4t2.y + v4t3.y) /3.0f)*1000;
			int middleZ = ((v4t1.z + v4t2.z + v4t3.z) /3.0f)*1000;
			int j = m_VoxelGridSize/2.0f + middleX / m_VoxelBlockSize;
			int k = m_VoxelGridSize/2.0f + middleY / m_VoxelBlockSize;
			int l = m_VoxelGridSize/2.0f + middleZ / m_VoxelBlockSize;
			...
			tempXplodePos[0].y = vertices[0].Position.y + (m_Timer * -0.01f*m_FallMultiply* abs( m_VoxelGridSize + (m_Gravity*(m_VoxelGridSize-k)) + m_AmountOfCrumble*(j - l * l)));
			tempXplodePos[0].z = vertices[0].Position.z;
			tempXplodePos[1].x = vertices[1].Position.x;
			tempXplodePos[1].y = vertices[1].Position.y + (m_Timer * -0.01f*m_FallMultiply* abs( m_VoxelGridSize + (m_Gravity*(m_VoxelGridSize-k)) + m_AmountOfCrumble*(j - l * l)));
			tempXplodePos[1].z = vertices[1].Position.z;
			tempXplodePos[2].x = vertices[2].Position.x;
			tempXplodePos[2].y = vertices[2].Position.y + (m_Timer * -0.01f*m_FallMultiply* abs( m_VoxelGridSize + (m_Gravity*(m_VoxelGridSize-k)) + m_AmountOfCrumble*(j - l * l)));
