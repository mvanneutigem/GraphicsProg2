...
bool DebugRenderer::m_RendererEnabled = false;//used to b tru
bool DebugRenderer::m_AddedFixedGeometry = false;
...
void DebugRenderer::AddFixedGeometry(ID3D11Device *pDevice, const std::vector<VertexPosCol>& dataArr)
{
	m_AddedFixedGeometry = true;
	//added this function
	for(UINT i = 0; i < dataArr.size(); ++i)
	{
		m_FixedLineList.push_back(dataArr[i]);
	}
...
void DebugRenderer::RemoveAllAddedFixedGeometry(ID3D11Device *pDevice)
{
	m_AddedFixedGeometry = false;
	CreateFixedLineList();
	CreateVertexBuffer(pDevice);
}
