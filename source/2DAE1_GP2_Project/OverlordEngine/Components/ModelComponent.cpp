...
void ModelComponent::Draw_ShadowMap(const GameContext& gameContext)
{
	if (!m_pMaterial)
	{
		Logger::LogWarning(L"ModelComponent::Draw_ShadowMap() > No Material!");
		return;
	}

	//yolo ill do it my own way

	gameContext.pShadowMapper->GetMaterial()->SetWorld(m_pGameObject->GetTransform()->GetWorld());
	gameContext.pShadowMapper->GetSkinnedMaterial()->SetWorld(m_pGameObject->GetTransform()->GetWorld());
	gameContext.pShadowMapper->GetMaterial()->SetEffectVariables(gameContext, this);
	gameContext.pShadowMapper->GetSkinnedMaterial()->SetEffectVariables(gameContext, this);

	gameContext.pShadowMapper->Draw(gameContext, m_pMeshFilter, m_pGameObject->GetTransform()->GetWorld());
}
...
{ 
	...
	//unbind if necessary
	m_pMaterial->Unbind(gameContext);
};
